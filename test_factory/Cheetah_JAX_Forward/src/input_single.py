...
r_max = 1e4 * UNITS.Mpc
r_samples = 1
r_array = jnp.array([1e2 * UNITS.Mpc])  #jnp.geomspace(r_min, r_max, r_samples)
...
p_max = 13 * Tnu_0
p_samples = 1
p_array = jnp.array([1 * Tnu_0]) #jnp.geomspace(p_min, p_max, p_samples)
u_ini_array = p_array / mass_fid

# Angular range
psi_samples = 1
psi_array = jnp.array([0]) #jnp.linspace(0.0, jnp.pi, psi_samples)
